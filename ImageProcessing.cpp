...
Image *ImageProcessing::convolution(float kernel[3][3], Image img) {
    ...
    Image *result = new Image(img.getFormat(), img.getWidth(), img.getHeight(), img.getDepth());
...
Image *ImageProcessing::toGrayscale(const Image &img) {
    float rWeight = 0.299;
    float gWeight = 0.587;
    float bWeight = 0.144;
    float weightedSum, Wr, Wg, Wb;
    Image *grayImg = new Image("P2", img.getWidth(), img.getHeight(), img.getDepth());
